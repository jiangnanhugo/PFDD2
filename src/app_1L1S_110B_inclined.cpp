...
#include "string.h"
#include "stdlib.h"
#include "app_1L1S_110B_inclined.h"
#include "fft.h"
#include "solve.h"
...
App1L1S110Binclined::App1L1S110Binclined(PFDD_C *pfdd_p, int narg, char **arg) :
  App(pfdd_p,narg,arg)
{
  ndouble = 0;
  timestep = 1.0;
  ...
  if(setup_flag == -1)
    setup_flag = 7; //default to 1L inclined fcc
  if(core_flag == -1)
...
App1L1S110Binclined::~App1L1S110Binclined()
{
  delete [] sites;
}
...
void App1L1S110Binclined::grow_app()
{
  darray = fft->xi;
}
...
void App1L1S110Binclined::init_app()
{
  delete [] sites;
...
/* ----------------------------------------------------------------------
   ...
   ------------------------------------------------------------------------- */
void App1L1S110Binclined::input_app(char *command, int narg, char **arg)
{
...
void App1L1S110Binclined::set_slip()
{
  int i, j, k;
  double s3, s2;
...
double App1L1S110Binclined::compute_mean_xi(int n, int flag)
{
  int lN1 = fft->local_x;
...
double App1L1S110Binclined::compute_stress(int n, int flag)
{
  int lN1 = fft->local_x;
  int lxs = fft->local_x_start;
  int N2 = fft->local_y;
...
double App1L1S110Binclined::compute_theta(int n, int flag)
{
  int lN1 = fft->local_x;
  int lxs = fft->local_x_start;
  int N2 = fft->local_y;
...
double App1L1S110Binclined::compute_strain(int n, int flag)
{
  int lN1 = fft->local_x;
  int lxs = fft->local_x_start;
  int N2 = fft->local_y;
...
void App1L1S110Binclined::stats(char *strtmp)
{
  char *strpnt = strtmp;
...
void App1L1S110Binclined::stats_header(char *strtmp)
{
  char *strpnt = strtmp;
