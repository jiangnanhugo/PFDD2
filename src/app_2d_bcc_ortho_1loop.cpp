...
#include "string.h"
#include "stdlib.h"
#include "app_2d_bcc_ortho_1loop.h"
#include "fft.h"
#include "solve.h"
...
App2Dbcc1LORTHO::App2Dbcc1LORTHO(PFDD_C *pfdd_p, int narg, char **arg) :
  App(pfdd_p,narg,arg)
{
  ndouble = 0;
  timestep = 1.0;
  ...
  if(setup_flag == -1)
    setup_flag = 8; //default to 1L ortho BCC
  if(core_flag == -1)
...
App2Dbcc1LORTHO::~App2Dbcc1LORTHO()
{
  delete [] sites;
}
...
/* ----------------------------------------------------------------------
   ...
   ------------------------------------------------------------------------- */
void App2Dbcc1LORTHO::input_app(char *command, int narg, char **arg)
{
...
void App2Dbcc1LORTHO::set_slip()
{
  ...
  fft->xn[0][0]= 1.0/sqrt(2);
  fft->xn[0][1]= -1.0/sqrt(2);
  fft->xn[0][2]= 0.0;
...
double App2Dbcc1LORTHO::compute_mean_xi(int n, int flag)
{
  int lN1 = fft->local_x;
...
double App2Dbcc1LORTHO::compute_stress(int n, int flag)
{
  int lN1 = fft->local_x;
  int lxs = fft->local_x_start;
  int N2 = fft->local_y;
...
double App2Dbcc1LORTHO::compute_strain(int n, int flag)
{
  int lN1 = fft->local_x;
  int lxs = fft->local_x_start;
  int N2 = fft->local_y;
...
void App2Dbcc1LORTHO::stats(char *strtmp)
{
  char *strpnt = strtmp;
...
void App2Dbcc1LORTHO::stats_header(char *strtmp)
{
  char *strpnt = strtmp;
