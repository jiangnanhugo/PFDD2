...
App3D2S::App3D2S(PFDD_C *pfdd_p, int narg, char **arg) :
  App(pfdd_p,narg,arg)
{
  ndouble = 0;
  timestep = 1.0;
  ...
  if(setup_flag == -1)
    setup_flag = 8; //default to 1LOrthoBCC
  if(core_flag == -1)
...
/* ----------------------------------------------------------------------
   ...
   ------------------------------------------------------------------------- */
void App3D2S::input_app(char *command, int narg, char **arg)
{
...
double App3D2S::compute_mean_xi(int n, int flag)
{
  int lN1 = fft->local_x;
...
double App3D2S::compute_stress(int n, int flag)
{
  int lN1 = fft->local_x;
  int lxs = fft->local_x_start;
  int N2 = fft->local_y;
...
double App3D2S::compute_strain(int n, int flag)
{
  int lN1 = fft->local_x;
  int lxs = fft->local_x_start;
  int N2 = fft->local_y;
...
void App3D2S::stats(char *strtmp)
{
  char *strpnt = strtmp;
...
void App3D2S::stats_header(char *strtmp)
{
  char *strpnt = strtmp;
