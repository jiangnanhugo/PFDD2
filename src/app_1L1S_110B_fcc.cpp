...
App1L1S110Bfcc::App1L1S110Bfcc(PFDD_C *pfdd_p, int narg, char **arg) :
  App(pfdd_p,narg,arg)
{
  ndouble = 0;
  timestep = 1.0;
...
/* ----------------------------------------------------------------------
   ...
   ------------------------------------------------------------------------- */
void App1L1S110Bfcc::input_app(char *command, int narg, char **arg)
{
...
void App1L1S110Bfcc::set_slip()
{
  ...
  fft->xb[0][0]= -1.0/sqrt(2.0);
  fft->xb[0][1]= 1.0/sqrt(2.0);
  fft->xb[0][2]= 0.0;
...
double App1L1S110Bfcc::compute_mean_xi(int n, int flag)
{
  int lN1 = fft->local_x;
...
double App1L1S110Bfcc::compute_stress(int n, int flag)
{
  int lN1 = fft->local_x;
  int lxs = fft->local_x_start;
  int N2 = fft->local_y;
...
double App1L1S110Bfcc::compute_strain(int n, int flag)
{
  int lN1 = fft->local_x;
  int lxs = fft->local_x_start;
  int N2 = fft->local_y;
...
void App1L1S110Bfcc::stats(char *strtmp)
{
  char *strpnt = strtmp;
...
void App1L1S110Bfcc::stats_header(char *strtmp)
{
  char *strpnt = strtmp;
