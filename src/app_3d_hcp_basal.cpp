...
App3DhcpBasal::App3DhcpBasal(PFDD_C *pfdd_p, int narg, char **arg) :
  App(pfdd_p,narg,arg)
{
  ndouble = 0;
  timestep = 1.0;
...
//oflag changed by Claire 08/13/18 to account for different dislocation characters


  if (narg < 1) error->all(FLERR,"Illegal app_style command");

  if(setup_flag == -1)
    setup_flag = 15; //default to hcp basal
  if(core_flag == -1)
...
/* ----------------------------------------------------------------------
   ...
   ------------------------------------------------------------------------- */
void App3DhcpBasal::input_app(char *command, int narg, char **arg)
{
...
double App3DhcpBasal::compute_mean_xi(int n, int flag)
{
  int lN1 = fft->local_x;
  ...
  // These are global indeces
  //  we will need local ones
  //  otherwise we can just use fft_fftw_slab
  //  TO DO
...
double App3DhcpBasal::compute_stress(int n, int flag)
{
  int lN1 = fft->local_x;
  int lxs = fft->local_x_start;
  int N2 = fft->local_y;
...
double App3DhcpBasal::compute_strain(int n, int flag)
{
  int lN1 = fft->local_x;
  int lxs = fft->local_x_start;
  int N2 = fft->local_y;
...
void App3DhcpBasal::stats(char *strtmp)
{
  char *strpnt = strtmp;
...
void App3DhcpBasal::stats_header(char *strtmp)
{
  char *strpnt = strtmp;
