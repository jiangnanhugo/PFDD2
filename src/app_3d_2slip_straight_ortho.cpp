...
#include "string.h"
#include "stdlib.h"
#include "app_3d_2slip_straight_ortho.h"
#include "fft.h"
#include "solve.h"
...
App3D2SStORTHO::App3D2SStORTHO(PFDD_C *pfdd_p, int narg, char **arg) :
  App(pfdd_p,narg,arg)
{
  ndouble = 0;
  timestep = 1.0;
  ...
  if(setup_flag == -1)
    setup_flag = 10; //default to straight ortho
  if(core_flag == -1)
...
/* ----------------------------------------------------------------------
   ...
   ------------------------------------------------------------------------- */
void App3D2SStORTHO::input_app(char *command, int narg, char **arg)
{
...
double App3D2SStORTHO::compute_mean_xi(int n, int flag)
{
  int lN1 = fft->local_x;
...
double App3D2SStORTHO::compute_stress(int n, int flag)
{
  int lN1 = fft->local_x;
  int lxs = fft->local_x_start;
  int N2 = fft->local_y;
...
double App3D2SStORTHO::compute_strain(int n, int flag)
{
  int lN1 = fft->local_x;
  int lxs = fft->local_x_start;
  int N2 = fft->local_y;
...
void App3D2SStORTHO::stats(char *strtmp)
{
  char *strpnt = strtmp;
...
void App3D2SStORTHO::stats_header(char *strtmp)
{
  char *strpnt = strtmp;
