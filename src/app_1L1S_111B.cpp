...
App1L1S111B::App1L1S111B(PFDD_C *pfdd_p, int narg, char **arg) :
  App(pfdd_p,narg,arg)
{
  ndouble = 0;
  timestep = 1.0;
  ...
  if(setup_flag == -1)
    setup_flag = 5; //default to 1L FCC
  if(core_flag == -1)
...
/* ----------------------------------------------------------------------
   ...
   ------------------------------------------------------------------------- */
void App1L1S111B::input_app(char *command, int narg, char **arg)
{
...
double App1L1S111B::compute_mean_xi(int n, int flag)
{
  int lN1 = fft->local_x;
...
double App1L1S111B::compute_stress(int n, int flag)
{
  int lN1 = fft->local_x;
  int lxs = fft->local_x_start;
  int N2 = fft->local_y;
...
double App1L1S111B::compute_strain(int n, int flag)
{
  int lN1 = fft->local_x;
  int lxs = fft->local_x_start;
  int N2 = fft->local_y;
...
void App1L1S111B::stats(char *strtmp)
{
  char *strpnt = strtmp;
...
void App1L1S111B::stats_header(char *strtmp)
{
  char *strpnt = strtmp;
